...
<%
  ...
  try {
    GetReportCustomerFee requestVos = (GetReportCustomerFee)JsonUtil.fromJson(request.getReader(), GetReportCustomerFee.class);
    if (requestVos == null) {
      return;
    }
    GetReportCustomerFeeResponse responseVos = new GetReportCustomerFeeResponse();
    responseVos = (GetReportCustomerFeeResponse) MGCConfig.listenerWebServer().sendRequestSyn(
        ExternalBodyId.GET_REPORT_CUSTOMER_FEE, JsonUtil.toJson(requestVos), responseVos, MGCAPIPublicValue.FM_WEB_TIMEOUT);
    if (responseVos == null) {
      responseVos = new GetReportCustomerFeeResponse();
